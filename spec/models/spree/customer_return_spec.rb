describe Spree::CustomerReturn, type: :model do
  ...
  describe "order in pending state",
    ...
    it 'does not accept the return' do
      allow(customer_return).to receive(:process_shipwire_return!).and_call_original

      customer_return.save

      expect(customer_return).to have_received(:process_shipwire_return!)
  ...
  context "order marked as shipped on shipwire",
    ...
    it 'post to shipwire' do
      ...
      expect(customer_return).to have_received(:process_shipwire_return!)
      expect(customer_return.errors.messages).to be_empty
      expect(customer_return.shipwire_id).not_to be_nil
    end

    context 'and a return was already reported',
            vcr: { cassette_name: 'spree/customer_return_entity_exists' } do
      before do
        Shipwire::Returns.new.create(customer_return.to_shipwire)
      end
    ...
    context "with multiple return items",
            vcr: { cassette_name: 'spree/customer_return_create_entity_multiple_return_item' } do
